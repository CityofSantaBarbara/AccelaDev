/*------------------------------------------------------------------------------------------------------/
| Program: CreateTylerDailyExtractAndSend.js  Trigger: Batch
| Client: Santa Barbara, CA
| Partner:  Silver Lining Solutions, Copywrite 2017
|
| Frequency: Run every night
|
| Desc: This batch script will create a Tyler Munis Extract report and FTP it for 
|       every record with a passed fire inspection dated today.
|
/------------------------------------------------------------------------------------------------------*/
/*  - use the following for Script Tester   
aa.env.setValue("showDebug","Y");
aa.env.setValue("dailyTranDate","01/17/2019");
aa.env.setValue("BatchJobName", "CreateTylerMunisDailytExtract-TEST");
aa.env.setValue("emailAddress","chad@slsgov.com");
...
aa.env.setValue("ftpFolder","Dev-TylerMunis");
*/
/*------------------------------------------------------------------------------------------------------/
|
...
var dailyTranDte = getParam("dailyTranDate");
...
if (emailAddress.length) {
	...
	logDebug("about to send an email FROM:"+scriptAgencyEmailFrom);
	logDebug("the to is:"+emailAddress);
	
	var emailSentOK = email(emailAddress,scriptAgencyEmailFrom, batchJobName + " Results", emailText);
	if (!emailSentOK) {
		logDebug("EMAIL COULD NOT BE SENT! >" + emailSentOK );
	}
	else {
		logDebug("EMAIL SENT. --> emailSentOK:"+emailSentOK);
		logDebug(emailText);
	}
}
...
function mainProcess() {
	...
	var trnDate = new String();
	
	if (dailyTranDte !=  null && dailyTranDte != "" ) {
		trnDate = dailyTranDte;
	} else {
		trnDate = curMonth + "/" + today.getDate() + "/" + today.getFullYear();
	}

	myParams.put ("p1Value", trnDate);
	tmpFTPFName = "AccelaDailyFinTrans_"+ today.getFullYear() + curMonth + today.getDate() + today.getHours() + today.getMinutes() + today.getSeconds() + ".csv";
	generateReportAndFTP("Tyler Munis Daily Extract", myParams,"Building", tmpFTPFName);
}
