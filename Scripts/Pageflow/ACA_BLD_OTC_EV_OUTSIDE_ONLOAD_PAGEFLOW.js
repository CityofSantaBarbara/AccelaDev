/*------------------------------------------------------------------------------------------------------/
| Program : ACA_BLD_OTC_EV_OUTSIDE_ONLOAD_PAGEFLOW.js
| Event   : ACA_BLD_OTC_EV_OUTSIDE_ONLOAD_PAGEFLOW Event
|
...
try {
   var hide = true;
    if ( String(AInfo["The electric vehicle charger will be located"]).toLowerCase() == 'outside of the building' )
      {
        hide = false;
        for(var x in AInfo){
            logDebug(x + " + " + AInfo[x]);
    ...
    showDebug = true;
    logDebug("An error has occurred in ACA_BLD_OTC_EV_OUTSIDE_ONLOAD_PAGEFLOW: Main function: " + err.message);
    logDebug(err.stack);
}
