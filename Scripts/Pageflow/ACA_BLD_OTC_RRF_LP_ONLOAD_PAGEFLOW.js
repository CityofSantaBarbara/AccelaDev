/*------------------------------------------------------------------------------------------------------/
| Program : ACA_BLD_OTC_RRF_LP_ONLOAD_PAGEFLOW.js
| Event   : ACA_BLD_OTC_RRF_LP_ONLOAD_PAGEFLOW Event
|
...
try {
   var hide = true;
    if ( String(AInfo["Owner Builder or Licesne Professional"]).toLowerCase() == 'license professional' )
         {
        hide = false;
        for(var x in AInfo){
            logDebug(x + " + " + AInfo[x]);
    ...
    showDebug = true;
    logDebug("An error has occurred in ACA_BLD_OTC_RRF_LP_ONLOAD_PAGEFLOW: Main function: " + err.message);
    logDebug(err.stack);
}
